...
public class Java8Concurrency {
	private static List<User> users = Arrays.asList(new User(1, "Steve", "Vai", 40), new User(4, "Joe", "Smith", 32),
			new User(3, "Steve", "Johnson", 57), new User(9, "Mike", "Stevens", 18),
			new User(10, "George", "Armstrong", 24), new User(2, "Jim", "Smith", 40),
	...
	private static void newJavaWay() {
		...
		System.out.println("printing all age");
		users.forEach(u -> System.out.println(u.age));

		System.out.println("******** Java 8 concurrency*************");
		double average = users.parallelStream().map(u -> u.age).mapToInt(x -> x).average().getAsDouble();
